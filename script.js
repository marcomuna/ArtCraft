...
(function () {
  ...
  links.forEach(l => {
    ...
    if (href === path || (path === '' && href === 'index.html') || href.split('/').pop() === path) {
...
function logoutUser()   { localStorage.removeItem(USER_KEY); window.location.href = 'auth.html'; }
function isAdmin()      { const u = getUser(); return u && u.isAdmin; }

function getUserAvatar(user) {
  if (user.avatar) return `<img src="${user.avatar}" style="width:28px;height:28px;border-radius:50%;object-fit:cover;border:1.5px solid var(--gold)">`;
...
function addToCart(product) {
  const cart = getCart();
  const idx  = cart.findIndex(i => i.id === product.id && i.frame === product.frame && i.size === product.size);
...
function removeFromCart(id, frame, size) {
  saveCart(getCart().filter(i => !(i.id === id && i.frame === frame && i.size === size)));
}

function updateQty(id, frame, size, qty) {
  const cart = getCart();
  const idx  = cart.findIndex(i => i.id === id && i.frame === frame && i.size === size);
  if (idx > -1) { if (qty < 1) cart.splice(idx, 1); else cart[idx].qty = qty; }
  saveCart(cart);
...
function toggleWish(product) {
  ...
  let added;
  if (idx > -1) { wish.splice(idx, 1); showToast('Removed from wishlist', 'info'); added = false; }
  else          { wish.push(product);  showToast('<i class="fas fa-heart"></i> Added to wishlist!', 'success'); added = true; }
  saveWish(wish);
  return added;
}
function isWished(id) { return getWish().some(i => i.id === id); }
...
function calcOrderTotals(cart) {
  ...
  const discount = subtotal > 5000 ? Math.round(subtotal * 0.20) : 0;
...
function updateOrderSummary(cart) {
  ...
  if (el('summary-subtotal'))  el('summary-subtotal').textContent  = '₹' + subtotal.toLocaleString();
  if (el('summary-discount'))  el('summary-discount').textContent  = discount > 0 ? '-₹' + discount.toLocaleString() + ' (20%)' : '—';
  if (el('summary-shipping'))  el('summary-shipping').textContent  = shipping === 0 ? 'FREE' : '₹' + shipping;
  if (el('summary-total'))     el('summary-total').textContent     = '₹' + total.toLocaleString();
  if (el('co-subtotal'))       el('co-subtotal').textContent       = '₹' + subtotal.toLocaleString();
  if (el('co-shipping'))       el('co-shipping').textContent       = shipping === 0 ? 'FREE' : '₹' + shipping;
...
function showToast(msg, type = 'info', duration = 3000) {
  ...
  setTimeout(() => { toast.style.animation = 'toastOut 0.35s ease forwards'; setTimeout(() => toast.remove(), 350); }, duration);
}
...
window.addEventListener('beforeinstallprompt', e => {
  ...
  if (banner && !localStorage.getItem('pwa_dismissed')) setTimeout(() => banner.classList.add('show'), 3000);
});
document.addEventListener('DOMContentLoaded', () => {
...
document.addEventListener('DOMContentLoaded', () => {
  const obs = new IntersectionObserver(entries => {
    entries.forEach(e => { if (e.isIntersecting) { e.target.src = e.target.dataset.src; e.target.removeAttribute('data-src'); obs.unobserve(e.target); } });
  }, { rootMargin: '200px' });
...
function renderWishlist() {
  ...
  if (wish.length === 0) {
    container.innerHTML = `<div style="text-align:center;padding:4rem 0;color:var(--text2);grid-column:1/-1">
      ...
      <a href="shop.html" class="btn btn-gold">Explore Shop</a></div>`; return;
  }
  container.innerHTML = wish.map(item => `
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (summaryEl) {
    if (!cart.length) { summaryEl.innerHTML = '<p style="color:var(--text2)">No items in cart</p>'; }
    else {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const frameAddon  = { 'Classic Black Frame': 200, 'Wooden Frame': 350, 'Premium Golden Frame': 550, 'None': 0 };
  let selectedFrame = 'None', selectedSize = 'A4';
  function updatePrice() {
    if (!priceEl) return;
    priceEl.textContent = '₹' + ((basePrices[selectedSize]||899) + (frameAddon[selectedFrame]||0)).toLocaleString();
  }
  document.querySelectorAll('[data-frame]').forEach(opt => {
    opt.addEventListener('click', () => {
      document.querySelectorAll('[data-frame]').forEach(o => o.classList.remove('active'));
      opt.classList.add('active'); selectedFrame = opt.dataset.frame; updatePrice();
    });
  });
  document.querySelectorAll('[data-size]').forEach(opt => {
    opt.addEventListener('click', () => {
      document.querySelectorAll('[data-size]').forEach(o => o.classList.remove('active'));
      opt.classList.add('active'); selectedSize = opt.dataset.size; updatePrice();
    });
  });
  updatePrice();
  ...
  if (addBtn) {
    addBtn.addEventListener('click', () => {
      const price = parseInt(priceEl ? priceEl.textContent.replace(/[₹,]/g,'') : 899);
      const size  = document.querySelector('[data-size].active')?.dataset.size || 'A4';
      const frame = document.querySelector('[data-frame].active')?.dataset.frame || 'None';
      const qty   = parseInt(document.getElementById('qty-display')?.textContent || 1);
      addToCart({ id:'p1', name:'Custom Pencil Portrait', price, image:'https://picsum.photos/seed/portrait-a4/400/530', category:'Pencil Portraits', size, frame, qty });
    });
  }
});
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (uploadInput && uploadPreview) {
    uploadInput.addEventListener('change', () => {
      ...
      reader.onload = e => { uploadPreview.innerHTML = `<img src="${e.target.result}" style="width:100%;height:100%;object-fit:cover;border-radius:var(--radius2)" alt="Preview">`; };
      reader.readAsDataURL(file);
    });
  }
  form.addEventListener('submit', e => {
    e.preventDefault();
    showToast('<i class="fas fa-check-circle"></i> Order placed! We\'ll WhatsApp you within 2 hours.', 'success', 6000);
  ...
  if (zone) {
    ...
    zone.addEventListener('drop', e => {
      ...
      if (file && uploadPreview) { const reader = new FileReader(); reader.onload = ev => { uploadPreview.innerHTML = `<img src="${ev.target.result}" style="width:100%;height:100%;object-fit:cover;border-radius:var(--radius2)" alt="Preview">`; }; reader.readAsDataURL(file); }
    });
  }
});
  ...
  document.querySelectorAll('.gallery-thumb').forEach(thumb => {
    thumb.addEventListener('click', () => {
      ...
      if (mainImg) {
        ...
        setTimeout(() => { mainImg.src = thumb.dataset.full || thumb.src; mainImg.style.opacity = '1'; mainImg.style.transform = 'scale(1)'; }, 200);
      }
    });
  });
});
...
const DEFAULT_PRODUCTS = [
  { id:'p1',  name:'A4 Pencil Portrait',      cat:'pencil',  size:'a4',     frame:'none', price:899,  oldPrice:1299, image:'https://picsum.photos/seed/portrait-a4/400/530',  badge:'Bestseller', category:'Pencil Portraits', stock:15 },
  { id:'p2',  name:'Mandala Circle Art',       cat:'mandala', size:'medium', frame:'none', price:1299, image:'https://picsum.photos/seed/mandala-round/400/530', category:'Mandala Art', stock:8 },
  { id:'p3',  name:'Lippan Mirror Decor',      cat:'lippan',  size:'medium', frame:'none', price:1599, badge:'New', image:'https://picsum.photos/seed/lippan-wall/400/530', category:'Lippan Art', stock:6 },
  { id:'p4',  name:'A3 Pencil Portrait',       cat:'pencil',  size:'a3',     frame:'none', price:1299, oldPrice:1799, image:'https://picsum.photos/seed/portrait-a3/400/530', category:'Pencil Portraits', stock:10 },
  { id:'p5',  name:'Couple Portrait A4',       cat:'pencil',  size:'a4',     frame:'black',price:1499, image:'https://picsum.photos/seed/couple-portrait/400/530', category:'Pencil Portraits', stock:12 },
  { id:'p6',  name:'Large Mandala Wall',       cat:'mandala', size:'large',  frame:'none', price:1899, image:'https://picsum.photos/seed/mandala-large/400/530', category:'Mandala Art', stock:5 },
  { id:'p7',  name:'Pet Portrait A4',          cat:'pencil',  size:'a4',     frame:'wood', price:999,  image:'https://picsum.photos/seed/pet-portrait/400/530', category:'Pencil Portraits', stock:20 },
  { id:'p8',  name:'Lippan Panel Large',       cat:'lippan',  size:'large',  frame:'none', price:2199, image:'https://picsum.photos/seed/lippan-panel/400/530', category:'Lippan Art', stock:4 },
  { id:'p9',  name:'A4 Portrait Golden Frame', cat:'pencil',  size:'a4',     frame:'gold', price:1449, badge:'Premium', image:'https://picsum.photos/seed/portrait-gold/400/530', category:'Pencil Portraits', stock:9 },
  { id:'p10', name:'Mandala Triptych',         cat:'mandala', size:'large',  frame:'none', price:2499, image:'https://picsum.photos/seed/mandala-trip/400/530', category:'Mandala Art', stock:3 },
  { id:'p11', name:'Family Portrait A3',       cat:'pencil',  size:'a3',     frame:'wood', price:1899, image:'https://picsum.photos/seed/family-portrait/400/530', category:'Pencil Portraits', stock:7 },
  { id:'p12', name:'Lippan Door Decor',        cat:'lippan',  size:'medium', frame:'none', price:1299, image:'https://picsum.photos/seed/lippan-door/400/530', category:'Lippan Art', stock:11 },
];

function getProducts()    { return JSON.parse(localStorage.getItem(PRODUCTS_KEY) || 'null') || DEFAULT_PRODUCTS; }
...
function makeImagesEditable() {
  ...
  document.querySelectorAll('img:not(.no-admin-edit)').forEach(img => {
    if (img.closest('.cursor-dot,.cursor-ring,.loading-screen,.review-avatar,.gallery-thumb')) return;
    ...
    if (wrap && !wrap.querySelector('.admin-edit-badge')) {
      ...
      badge.onclick = (e) => { e.stopPropagation(); adminEditImage(img); };
...
document.addEventListener('DOMContentLoaded', () => {
  if (isAdmin()) {
    ...
    banner.style.cssText = 'position:fixed;top:var(--nav-h);left:0;right:0;background:var(--gold);color:var(--bg);text-align:center;padding:6px;font-size:0.78rem;font-weight:600;z-index:9990;letter-spacing:0.08em';
    banner.innerHTML = '<i class="fas fa-crown"></i> ADMIN MODE — <a href="admin.html" style="color:var(--bg);text-decoration:underline">Go to Dashboard</a>';
